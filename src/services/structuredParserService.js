...
const STOPWORDS = new Set([
  '的', '了', '和', '与', '及', '对', '在', '中', '是', '为', '就', '并', '或', '及其',
  '以及', '根据', '关于', '有关', '相关', '工作', '管理', '要求', '开展', '推进', '实施', '问题',
  '进行', '方案', '制度', '报告', '办法', '规定', '流程', '公司', '部门', '本次', '其中'
])
...
function inferDepartment(fileName, text) {
  ...
  const patterns = [
    '调控', '配电', '营销', '规划', '通信', '运检', '安监', '财务', '设备', '发展', '调度'
  ]
...
function inferDocType(fileName, text) {
  ...
  if (/(纪要|总结|汇报)/.test(source)) return '报告'
...
class StructuredParserService {
  async parseDocument(filePath, { documentId, fileName, createdAt }) {
    ...
    const documentMeta = {
      ...
      docType: inferDocType(fileName, fullText),
      authorityLevel: inferAuthorityLevel(inferDocType(fileName, fullText), fileName),
  ...
  buildPage(page, documentMeta) {
    ...
    const pageKeywords = buildKeywordList(page.text, 8)
  ...
  extractBlocks(pageText, pageNumber, documentMeta) {
    ...
    const flushParagraph = () => {
      ...
      const startOffset = paragraphStart
    ...
    for (let index = 0; index < lines.length; index += 1) {
      ...
      const type = this.classifyLine(line, index, lines.length)
  ...
  classifyLine(line, lineIndex, totalLines) {
    if (/^(第?\s*\d+\s*页|\d+\/\d+|\d+)$/i.test(line)) return 'pageNumber'
    if (/^(图|figure)\s*[0-9一二三四五六七八九十]/i.test(line)) return 'figureCaption'
    if (/^(表|table)\s*[0-9一二三四五六七八九十]/i.test(line)) return 'table'
    ...
    if (
      line.length <= ragConfig.parser.titleMaxLength &&
      (/^[一二三四五六七八九十\d]+[、.．)\s]/.test(line) || /^[（(]?[一二三四五六七八九十\d]+[)）]/.test(line) || !/[。；，]$/.test(line))
    ) {
      return 'title'
    }
  ...
  classifyLine(line, lineIndex, totalLines) {
    if (/^(绗?\s*\d+\s*椤祙\d+\/\d+|\d+)$/i.test(line)) return 'pageNumber'
    if (/^(鍥緗figure)\s*[0-9涓€浜屼笁鍥涗簲鍏竷鍏節鍗乚/i.test(line)) return 'figureCaption'
    if (/^(琛▅table)\s*[0-9涓€浜屼笁鍥涗簲鍏竷鍏節鍗乚/i.test(line)) return 'table'
    if (/[=鈮犫増鈮も墺鈭戔垰]/.test(line) || /[A-Za-z]\s*=\s*[A-Za-z0-9]/.test(line)) return 'formula'
    if (/\|/.test(line) || /\t/.test(line) || /(\S+\s{2,}\S+\s{2,}\S+)/.test(line)) return 'table'
    if (lineIndex === 0 && line.length <= 18 && !/[。；！？]/.test(line)) return 'header'
    if (lineIndex === totalLines - 1 && line.length <= 18 && !/[。；！？]/.test(line)) return 'footer'
    if (this.isLikelyTitle(line)) return 'title'
  ...
  markRepeatedHeadersAndFooters(pages) {
    ...
    for (const page of pages) {
      ...
      if (first && (firstCounts.get(first.text) || 0) >= 2) {
        first.type = 'header'
      }

      if (last && (lastCounts.get(last.text) || 0) >= 2) {
        last.type = 'footer'
      }
    }
  }
  ...
  buildChunks(parentBlocks, pages, documentMeta) {
    ...
    for (const parent of parentBlocks) {
      ...
      for (let index = 0; index < parentChunks.length; index += 1) {
        ...
        const contextualText = `${chunkContext}\n\n正文:\n${rawText}`.trim()
  ...
  splitText(text) {
    ...
    if (clean.length <= ragConfig.chunking.maxChars) {
      return [clean]
    }
...
module.exports = new StructuredParserService()
